...
from unravel.config import PROVIDER_ENV_KEYS, UnravelConfig, normalize_provider
...
def get_provider(config: UnravelConfig) -> BaseProvider:
    ...
    metadata) sees the resolved name instead of ``"auto"``. Legacy aliases
    (e.g. ``"anthropic"``) are normalized to their canonical id.
    """
    ...
    match config.provider:
        ...
        case _:
            supported = ["auto", "claude-api", "claude-cli"]
            raise ValueError(
...
def _auto_detect(config: UnravelConfig) -> str:
    ...
    if shutil.which(config.claude_cli.path):
        return "claude-cli"
    if os.environ.get(PROVIDER_ENV_KEYS["claude-api"]):
        return "claude-api"
    return "claude-cli"
